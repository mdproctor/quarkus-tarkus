...
`quarkus-tarkus-ledger` originally contained all ledger infrastructure —
`LedgerEntry`, `LedgerAttestation`, `ActorTrustScore`, `TrustScoreComputer`,
`LedgerHashChain`, `LedgerConfig`, and their repositories — as Tarkus-specific
classes. CaseHub and Qhorus will need the same accountability infrastructure.
Keeping it inside Tarkus would force both to depend on Tarkus to get it.

## Decision Drivers

* CaseHub and Qhorus need command/event ledgers, hash chains, and trust scores
  without depending on Tarkus
* Each ecosystem extension should own only what is specific to it
* The `WorkItem` concept is Tarkus-specific; the ledger pattern is not

## Considered Options

* **Option A** — Extract generic ledger infrastructure to `io.quarkiverse.ledger:quarkus-ledger`; Tarkus provides `WorkItemLedgerEntry` as a subtype
* **Option B** — Copy the ledger classes into each downstream project (CaseHub, Qhorus) independently
* **Option C** — Keep everything in `quarkus-tarkus-ledger`; CaseHub and Qhorus depend on Tarkus to get the ledger

## Decision Outcome

Chosen option: **Option A**, because it gives CaseHub and Qhorus a shared
accountability layer without coupling them to Tarkus, and avoids duplicating
...
* `WorkItemLedgerEntry` (extends `LedgerEntry`) uses JPA JOINED inheritance —
  the Tarkus subtype adds only `commandType` and `eventType`; all common fields
...
* `quarkus-ledger` is a separate local project (`~/claude/quarkus-ledger/`)
  ...
  `quarkus-tarkus-ledger` — documented in CLAUDE.md
...
* ✅ CaseHub and Qhorus can adopt it without depending on Tarkus
* ✅ Tarkus-specific fields isolated in the subtype
...
### Option C — Keep in quarkus-tarkus-ledger; downstream depends on Tarkus

* ✅ No new library
* ❌ CaseHub and Qhorus take a Tarkus dependency they don't conceptually need
* ❌ Creates a circular dependency risk when Tarkus integrates with CaseHub

## Links

* Issue: #49 — Migrate quarkus-tarkus-ledger to use quarkus-ledger as shared base library
* `quarkus-ledger` source: `~/claude/quarkus-ledger/`
* `WorkItemLedgerEntry`: `quarkus-tarkus-ledger/src/main/java/io/quarkiverse/tarkus/ledger/model/WorkItemLedgerEntry.java`
