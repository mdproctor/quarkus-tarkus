...
Also requires `quarkus-work` (the core extension) and a configured datasource.
...
## Relationship to quarkus-work

`quarkus-work-flow` depends on `quarkus-work` — it does not modify the core extension.
`WorkItemService` and the CDI event infrastructure come from the core. If you use
`quarkus-work` without this module, none of these classes are on the classpath.
