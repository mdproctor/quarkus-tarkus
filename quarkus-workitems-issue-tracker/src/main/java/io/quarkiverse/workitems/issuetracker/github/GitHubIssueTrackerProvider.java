package io.quarkiverse.workitems.issuetracker.github;
...
import io.quarkiverse.workitems.issuetracker.spi.ExternalIssueRef;
import io.quarkiverse.workitems.issuetracker.spi.IssueTrackerException;
import io.quarkiverse.workitems.issuetracker.spi.IssueTrackerProvider;
 ...
 * <p>
 * Set {@code quarkus.workitems.issue-tracker.github.token} to a PAT with
    ...
    public Optional<String> createIssue(final UUID workItemId, final String title, final String body) {
        final String repo = config.defaultRepository()
                .orElseThrow(() -> new IssueTrackerException(
                        "quarkus.workitems.issue-tracker.github.default-repository is required for createIssue"));
        ...
        if (response.statusCode() == 401 || response.statusCode() == 403) {
            throw new IssueTrackerException(
                    "GitHub auth failure (" + response.statusCode() + ") for " + context +
                            ". Check quarkus.workitems.issue-tracker.github.token");
        }
        ...
        final String repo = config.defaultRepository()
                .orElseThrow(() -> new IssueTrackerException(
                        "Cannot resolve bare issue ref '" + externalRef +
                                "': quarkus.workitems.issue-tracker.github.default-repository is not set"));
